...
%%       
        ...
        j = randperm(nVar, nMu);
        x1=x.Position;
        x1(j) = x.Position(j) + sigma(j) .* randn(1, nMu);
%%        
        nMu = ceil(mu*nVar);
        j = randperm(nVar, nMu); % Đã đổi thành randperm
        y1 = y.Position;    
        y1(j) = y.Position(j) + sigma(j) .* randn(1, nMu);
%%
        nMu = ceil(mu*nVar);
        j = randperm(nVar, nMu); % Đã đổi thành randperm
        z1 = z.Position;   
        z1(j) = z.Position(j) + sigma(j) .* randn(1, nMu);
%%                
